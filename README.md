...
1. Lets start by implementating the basics for the basket reducer, open `src/store/modules/basket.js`. 
  1. (Optional) Enabled `const ENABLE_EXERCISE_1 = true;`
  2. Implement the tasks for adding and removing a product from the basket
...
1. Lets enable our basket now.
  1.  Create a shopping card container in `src/containers`
  2.  Let this container wrap the basket container and connect
  3.  Implement this in the `<App />`
  4.  Seperate the Header component from the basket button (`src/components/header`), let the header connect with the store and pass the amount of basket products to this button.
